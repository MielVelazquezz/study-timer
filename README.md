...
- **Calendar & agenda**: open a month view with colored dots showing which subjects you studied each day, plus a simple per-day agenda (add, check off, and remove planned items) - click any day to see the breakdown and plan ahead.
- **Obsidian colors**: the plugin's highlight follows Obsidian's accent color and the light/dark theme.
- **Languages**: the interface is available in Portuguese, English and Spanish, selectable in Settings -> Study Timer -> Language.
...
2. Type a subject in *New subject*, pick a color and click **Add**. Click a subject to select it, the colored dot to change its color, and the **×** to remove it (history that was already recorded is kept).
3. Press ▶ to start and ❚❚ to pause. Only whole minutes are saved.
4. Switch between **Stopwatch** and **Pomodoro** at the top of the panel.
